...
export async function resolveVizData(projetoId, conteudo) {
  ...
  // Agregado do TIME = só colaboradores. Sem esse filtro, os markers
  // `*_time` misturariam sócios + colaboradores, deturpando a leitura
  // coletiva (time do GSIM tem 2 sócios de DISC distinto que puxariam
  // a média dos 10 colaboradores).
  const cisColab = (cisAssessments || []).filter(
    c => c.papel === 'colaborador' || c.papel === 'colaboradores',
  );
  const agregado = cisColab.length > 0 ? aggregateCisByProject(cisColab) : null;
...
async function fetchCisAssessmentsDoProjeto(projetoId) {
  ...
  // Enriquece com papel via respondentes — cis_assessments não tem a
  // coluna, e os resolvers `*_time` precisam filtrar só colaboradores.
