...
DADOS: Nome: ${nome}, Perfil: ${sc.profile || '—'}, DISC Natural: D=${safeDisc.D ?? '—'} I=${safeDisc.I ?? '—'} S=${safeDisc.S ?? '—'} C=${safeDisc.C ?? '—'}, DISC Adaptado: D=${safeDA.D ?? '—'} I=${safeDA.I ?? '—'} S=${safeDA.S ?? '—'} C=${safeDA.C ?? '—'}, Liderança: ${Object.entries(safeLead).map(([k, v]) => k + '=' + v).join(', ') || '—'}, Top: ${top3}, Gap: ${gap3}
