...
export const AIRouter = {
  ...
  MODELS: {
    ...
    "claude-sonnet": { id: "claude-3-5-sonnet-20240620", provider: "anthropic" },
    "claude-haiku": { id: "claude-3-haiku-20240307", provider: "anthropic" },
  ...
  async callModel(systemPrompt, messages, options = {}) {
    ...
    console.log(`[AIRouter] calling ${provider} with model ${modelId}`);

    if (provider === "gemini") {
      return this._callGemini(systemPrompt, messages, modelId, maxTokens, temperature);
  ...
  async _callGemini(systemPrompt, messages, modelId, maxTokens, temperature) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) throw new Error("GEMINI_API_KEY is missing from .env.local");

    // As APIs mais novas do Gemini usam v1beta e models/
