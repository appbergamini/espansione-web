...
export const Agent_05_BuscaWeb = {
  ...
  async enrichContext(context) {
    ...
    for (const f of socios) {
      ...
      // concorrentes — procura em chaves comuns
      const rawCandidates = [
      ...
      for (const v of rawCandidates) {
        if (!v) continue;
        if (Array.isArray(v)) v.forEach(x => x && concorrentesSet.add(String(x).trim()));
        else if (typeof v === 'string') {
          v.split(/[,;\n]/).map(s => s.trim()).filter(Boolean).forEach(x => concorrentesSet.add(x));
        }
      }
    }
