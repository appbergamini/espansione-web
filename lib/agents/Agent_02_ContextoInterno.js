...
export const Agent_02_ContextoInterno = {
  ...
  getSystemPrompt() {
    return [
      ...
      '- Colaboradores: intake_colaboradores ANÔNIMO consolidado (NUNCA identificado por nome/e-mail/token/cargo específico), transcrições de entrevistas/focus groups com colaboradores voluntários (se houver), CIS estruturado do time (agregado).',
      ...
      'onde {nome-do-socio-slug} é DERIVADO EXATAMENTE do campo "SÓCIO N: {nome}" do input, sem adicionar, completar ou inferir partes do nome. Regra de slug: lowercase, sem acentos (normalização NFD), espaços → hífen, remover pontuação. Exemplos:',
      '  - Input "SÓCIO 1: Rafael Menezes"  → marker: <!-- VIZ:radar_disc_socio:rafael-menezes -->',
      '  - Input "SÓCIO 2: Ana Costa"        → marker: <!-- VIZ:radar_disc_socio:ana-costa -->',
      '  - Input "SÓCIO 3: João Silva Souza" → marker: <!-- VIZ:radar_disc_socio:joao-silva-souza -->',
      'PROIBIDO inventar sobrenomes/nomes do meio que não estejam literalmente no input — o slug precisa casar com o nome armazenado em cis_assessments.nome para o radar ser renderizado.',
  ...
  getUserPrompt(context) {
    ...
    parts.push(`Entrevistas/focus groups de colaboradores: ${entColab.length}`);
    parts.push(`Testes de Posicionamento: ${posic.length}`);
    parts.push(`CIS registrados: ${cis.length}`);
    ...
    if (entColab.length > 0) {
      parts.push('');
      parts.push('=== TRANSCRIÇÕES DE ENTREVISTAS/FOCUS GROUPS COM COLABORADORES VOLUNTÁRIOS ===');
      parts.push('NOTA: essas entrevistas têm identificação do voluntário (que deu opt-in explícito na Bloco 8). Ao citar no output, usar codinome ou cluster amplo — NUNCA nome próprio.');
      entColab.forEach((f, i) => {
        parts.push('');
        parts.push(`--- ENTREVISTA/FOCUS ${i + 1} ---`);
