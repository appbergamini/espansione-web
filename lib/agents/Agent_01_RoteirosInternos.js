import { AC_INVESTIGACAO_SIMULTANEA, AC_ONDAS, AC_PRINCIPIOS } from './_anaCoutoKB';
...
export const Agent_01_RoteirosInternos = {
  ...
  getUserPrompt(context) {
    ...
    const nomesSocios = socios.map((f, i) => {
      ...
      return f.respondente || r._respondente_nome || `Sócio ${i + 1}`;
    });
    parts.push('════════════════════════════════════════════════════════════════');
    ...
    parts.push('  × referenciar terceiros não listados (ex.: "Fernando (COO)", "Eduardo", "o diretor financeiro X")');
    parts.push('  × simular dinâmica com sócios fora da lista acima');
    parts.push('  × atribuir cargos/papéis (COO, CFO, Diretor) que não vieram do formulário');
    parts.push('Qualquer nome fora desta lista = alucinação = quebra a entrevista real.');
    parts.push('════════════════════════════════════════════════════════════════');
    parts.push('');
    ...
    for (const f of socios) {
      const r = f.respostas_json || {};
      const nome = f.respondente || r._respondente_nome || 'anônimo';
      const email = r._respondente_email || f.respondente_email || null;
      const cisRec = findCis(email, nome);
      const posRec = findPosicionamento(email, nome);
      parts.push('');
