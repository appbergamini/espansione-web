export const Agent_08_Visual = {
  ...
  getUserPrompt: function(context) {
    ...
    parts.push("=== INPUT PARA BRIEFING VISUAL ===");
    parts.push("");

    // ── Output 6 — Plataforma de Branding (completo) ────────────────
    ...
    // ── Output 7 — Guia de Identidade Verbal (completo) ─────────────
    parts.push("--- GUIA DE IDENTIDADE VERBAL (Output 7 — completo) ---");
    if (context.previousOutputs && context.previousOutputs[7]) {
      var out7 = context.previousOutputs[7];
      if (out7.resumo_executivo) {
        parts.push("[Resumo Executivo]");
        parts.push(out7.resumo_executivo);
        parts.push("");
      }
      if (out7.conteudo) {
        parts.push("[Conteúdo]");
        parts.push(out7.conteudo);
        parts.push("");
      }
      if (out7.conclusoes) {
        parts.push("[Conclusões]");
        parts.push(out7.conclusoes);
        parts.push("");
      }
    } else {
      parts.push("Output 7 não disponível.");
      missing.push("Output 7 (Guia de Identidade Verbal)");
    }
    parts.push("");
    ...
    // ── Instrução final ───────────────────────────────────────────────
    parts.push("---");
    parts.push("Crie o Output 8 (Briefing de Identidade Visual) conforme o formato especificado.");
    parts.push("Garanta coerência entre tom de voz (Output 7) e direção visual.");
    parts.push("Cada recomendação deve ser justificada pela Plataforma (Output 6).");
