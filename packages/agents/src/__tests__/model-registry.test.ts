...
test('modo economical usa Gemini 3 Flash', () => {
  const model = resolveModelForAgencyStep({
    agentId: 'approver',
  ...
  assert.equal(model.model_id, 'gemini-3-flash-preview');
});
...
test('use_agent_defaults usa modelo recomendado por agente', () => {
  ...
  assert.equal(getDefaultModelForAgent('channel_adapter').model_id, 'gemini-3-flash-preview');
});
...
test('override_single_agent aplica override apenas no agente escolhido', () => {
  const override = resolveModelForAgencyStep({
    ...
    modelSelection: {
      ...
      agent_overrides: [{ agent_id: 'editor', model_id: 'gemini-3-flash-preview' }],
    },
  });
  ...
  assert.equal(override.model_id, 'gemini-3-flash-preview');
...
test('getEconomicalModelForAgent sempre retorna Gemini 3 Flash', () => {
  assert.equal(getEconomicalModelForAgent('visual_director').model_id, 'gemini-3-flash-preview');
});
