...
const L=`<div style="background:#fff;border-radius:8px;padding:6px 12px;display:inline-flex;align-items:center"><img src="/brand/logo.png" alt="Espansione" style="height:32px;width:auto"></div>`;
...
function rRes(){
  ...
  <div class="sg" style="text-align:center;padding:20px 0 24px"><div style="margin-bottom:16px">${L}</div>
  <p class="tag" style="margin-bottom:6px;font-size:11px;letter-spacing:2px">Seu perfil comportamental</p>
...
async function generateReport(sc){
  ...
  function hdr(p){if(p<2)return;pdf.setFillColor(...navy);pdf.rect(0,0,W,24,'F');pdf.setFillColor(...teal);pdf.rect(0,24,W,0.4,'F');pdf.setFont('helvetica','bold',10);pdf.setTextColor(...teal);pdf.text('ESPANSIONE',ML,14);pdf.setFont('helvetica','normal',7);pdf.setTextColor(...white);pdf.text(G.ud.name,W-ML,14,{align:'right'})}
  function ftr(p){pdf.setFont('helvetica','normal',5.5);pdf.setTextColor(...grayD);pdf.text('Espansione © 2026',ML,H2-8);pdf.text('Pág. '+p,W-ML,H2-8,{align:'right'})}
  ...
  pdf.setFont('helvetica','bold',26);pdf.setTextColor(...teal);pdf.text('ESPANSIONE',ML,52);
  ...
  pdf.text('Documento confidencial — Espansione © 2026',ML,H2-10);
  ...
  pdf.save(`Espansione_Perfil_${G.ud.name.replace(/\s+/g,'_')}.pdf`);
