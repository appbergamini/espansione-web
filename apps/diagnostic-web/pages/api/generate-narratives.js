...
export default async function handler(req, res) {
  ...
  try {
    const { prompt, projetoId, email } = req.body;
    if (!prompt) {
      return res.status(400).json({ success: false, error: 'prompt é obrigatório' });
    }
    if (!projetoId || !email) {
      return res.status(400).json({ success: false, error: 'projetoId e email são obrigatórios' });
    }
    ...
    if (!participante) {
      return res.status(403).json({ success: false, error: 'E-mail não autorizado para este projeto.' });
    }
