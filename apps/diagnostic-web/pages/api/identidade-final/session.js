// /api/identidade-final/session — PÚBLICO, por token de link (por público)
// GET  ?token=&rid=        → perguntas do formulário + respostas salvas
// POST { token, rid?, answers } → upsert das respostas (autosave)
//
// Sócios: 1 respondente por assessment. Colaboradores/Clientes: multi-
// respondente — cada pessoa é um respondente; o client guarda o rid.

import { supabaseAdmin } from '../../../lib/supabaseAdmin';
import { montarFormulario } from '../../../lib/identidade-final/forms';
...
function toCells(value) {
  ...
  if (value != null && typeof value === 'object') return { value_num: null, value_text: null, value_json: value };
...
function slimPerguntas(perguntas) {
  return perguntas.map((p) => ({
    ...
    classificacao: p.classificacao, regra_condicional: p.regra_condicional || null,
  }));
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('[identidade-final/session]', e);
