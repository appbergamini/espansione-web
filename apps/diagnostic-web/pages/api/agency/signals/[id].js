import { getServerUser } from '../../../../lib/getServerUser';
import { supabaseAdmin } from '../../../../lib/supabaseAdmin';
import { requireAgencyUser } from '../../../../lib/agency/runtime';
import {
  archiveAgencySignal,
  convertAgencySignalToLearningSuggestion,
  dismissAgencySignal,
  markAgencySignalReviewed,
} from '../../../../lib/agency/agencySignals';

export default async function handler(req, res) {
  const { user } = await getServerUser(req, res);
  if (!user) return res.status(401).json({ success: false, error: 'Não autenticado' });
  if (!supabaseAdmin) return res.status(500).json({ success: false, error: 'Supabase admin indisponível' });

  try {
    await requireAgencyUser(supabaseAdmin, user);
    const { id } = req.query || {};

    if (req.method === 'PATCH' || req.method === 'POST') {
      const { action } = req.body || {};
      if (action === 'review') {
        const signal = await markAgencySignalReviewed(supabaseAdmin, id);
        return res.status(200).json({ success: true, signal });
      }
      if (action === 'dismiss') {
        const signal = await dismissAgencySignal(supabaseAdmin, id);
        return res.status(200).json({ success: true, signal });
      }
      if (action === 'archive') {
        const signal = await archiveAgencySignal(supabaseAdmin, id);
        return res.status(200).json({ success: true, signal });
      }
      if (action === 'convert_to_learning') {
        const result = await convertAgencySignalToLearningSuggestion(supabaseAdmin, id, req.body || {});
        return res.status(200).json({ success: true, ...result });
      }
      return res.status(400).json({ success: false, error: 'action inválida' });
    }

    if (req.method === 'DELETE') {
      const signal = await archiveAgencySignal(supabaseAdmin, id);
      return res.status(200).json({ success: true, signal });
    }

    return res.status(405).json({ success: false, error: 'Method not allowed' });
  } catch (error) {
    console.error('[agency/signals/:id]', error);
    return res.status(error.statusCode || 500).json({ success: false, error: error.message });
  }
}
