    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.2rem' }}>
      ...
      <div>
        ...
        <div style={{ ...c.grid2, marginTop: '0.7rem' }}>
          ...
          <ModuleCard titulo="🧬 Mapa do Crescimento Integrado Estratégico" status={s.identity.status} href={`/adm/${projetoId}`} cta="Gerenciar identidade"
          ...
          <ModuleCard titulo="📘 Relatório PDF" status={s.deliverables.pdf_identidade_blocked ? STATUS.BLOCKED : STATUS.NOT_STARTED}
            linhas={[['Identidade', s.deliverables.pdf_identidade_blocked ? 'Bloqueado' : 'Pode gerar']]} hint={s.deliverables.pdf_identidade_blocked ? 'Conclua as fontes obrigatórias do Mapa do Crescimento Integrado Estratégico para liberar.' : 'Ao gerar o PDF, a Leitura Guiada é liberada junto.'} />
          <ModuleCard titulo="📖 Leitura Guiada" status={STATUS.BLOCKED} linhas={[['Público', 'Sócios']]} hint="Liberada automaticamente junto com o PDF do Mapa do Crescimento Integrado Estratégico." />
...
export function FormulariosTab({ cockpit, projetoId }) {
  const s = cockpit.summary;
  const linhas = [
    { label: 'Mapa do Crescimento Integrado Essencial — questionário', status: s.maturity.status, href: null },
...
export function EntregaveisTab({ cockpit, projetoId }) {
  ...
  const items = [
    { label: 'PDF do Mapa do Crescimento Integrado Essencial', ok: d.pdf_maturidade, href: null },
    { label: 'PDF do Mapa do Crescimento Integrado Estratégico', ok: !d.pdf_identidade_blocked, blocked: d.pdf_identidade_blocked },
