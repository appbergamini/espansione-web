...
export default function Secao7_OptInEntrevista({ dados, atualizar, erros }) {
  ...
  return (
    <section>
      ...
      <Campo label="34. Você toparia participar de uma conversa de ~45 minutos?">
        <RadioGroup
      ...
      {topa && (
        <LegendSection titulo="Dados para contato">
          <Campo label={<>35. Melhor forma de contato <RequiredMark/></>} erro={erros.s7_canal_preferido}>
          ...
          <Campo id="s7_contato" label={<>36. WhatsApp / e-mail / telefone para contato <RequiredMark/></>} erro={erros.s7_contato}>
            <input className="form-input" type="text" id="s7_contato"
              value={dados.s7_contato || ''} onChange={e => atualizar('s7_contato', e.target.value)} />
          </Campo>

          <Campo label={<>37. Melhor horário para conversar <RequiredMark/></>} erro={erros.s7_horario}>
