...
export const Agent_04_ContextoExterno = {
  // FIX.12 — getUserPrompt injeta previousOutputs[3] (roteiro) manualmente.
  ...
  getSystemPrompt() {
    return [
      ...
      // Particularidade do Agente 4: codinomes (E1, E2…) preservados no JSON pra
      // rastreabilidade interna entre evidências e perfis. Anonimato externo (sem
      // nome real, sem e-mail) é mantido.
      ...
      '- schema_version sempre "2.0", agent_id sempre 4.',
      '- caveat_amostra é a versão técnica do caveat (mesma do topo da Parte A) — texto byte-a-byte do <conteudo>.',
      '- perfil_amostra: um item por entrevistado, mantendo o codinome (E1, E2…) que você usou no analítico. Anonimato externo preservado — nada de nome ou e-mail.',
      '- subsegmentacao_icp: cada cluster que você identificou na seção 4. Se não há variação clara, emita array vazio [] e adicione a _gaps.',
      '- experiencia_atual.pontos_fortes e pontos_fracos: listas curtas dos achados das seções 5.3 e 5.4 (texto resumido, não citações inteiras).',
      '- marca_atual_vs_ideal: as 4 zonas da seção 7.2 (já_entrega, está_perto, está_longe, ninguém_entrega) viram arrays de strings.',
      '- dados_quantitativos: cada métrica do <conteudo> (NPS médio, scores por dimensão) com sua leitura curta. Sempre carregue a ressalva no campo leitura se aplicável.',
      '- ida_ve: array de strings — listagem dos 5–7 itens de cada quadrante.',
      '- hipoteses_direcionais: cada hipótese da seção 13 vira { dimensao, hipotese }. dimensao normalizada: "negocio" | "marca" | "comunicacao" (minúsculo, sem acento). hipotese mantém o prefixo "A VE sugere que…" do <conteudo>.',
      ...
      '- ve.percepcao_geral: string não-vazia (tese central da VE)',
      '- ve.ida_ve: objeto com impulsionadores, detratores, aceleradores (arrays podem ter conteúdo ou estar vazios, mas as chaves existem)',
      '',
      'FORMATO:',
      '',
      '<brand_memory_export>',
      ...
      '  "agent_id": 4,',
      '  "ve": {',
      '    "caveat_amostra": "Esta escuta capturou exclusivamente clientes ativos próximos do ICP (N = 5)…",',
      '    "metodologia": {',
      '      "icp_descricao": "Empresas B2B de grande porte ou alto crescimento com necessidades de comunicação complexa.",',
      '      "n_respondentes": 5,',
      '      "instrumento": "Formulários de percepção externa + entrevistas de aprofundamento",',
      '      "periodo": "Abril de 2026"',
      '    },',
      '    "perfil_amostra": [',
      '      {',
      '        "codinome": "E1",',
      '        "subperfil": "Indústria (Petroquímica)",',
      '        "tempo_relacao": "6 anos",',
      '        "ticket_medio": "R$ 280k/mês",',
      '        "nps_equivalente": 9',
      '      }',
      '    ],',
      '    "subsegmentacao_icp": [',
      '      {',
      '        "cluster_nome": "Os Legados",',
      '        "representantes": ["E1","E2"],',
      '        "descricao": "Clientes de longo prazo em setores tradicionais.",',
      '        "percepcao_marca": "Valorizam a marca como seguro de qualidade; aceitam a lentidão em troca da senioridade."',
      '      }',
      '    ],',
      '    "percepcao_geral": "A marca é percebida como Boutique de Alta Especialidade que enfrenta crise de expectativa…",',
      '    "primeiro_contato": "A atração inicial é estética e estratégica…",',
      '    "experiencia_atual": {',
      '      "pontos_fortes": ["Acabamento visual de nível cinematográfico","Entendimento de temas sensíveis","Olhar editorial proprietário"],',
      '      "pontos_fracos": ["Comunicação intermitente","Integração falha entre unidades","Reatividade"]',
      '    },',
      '    "marca_atual_vs_ideal": {',
      '      "ja_entrega": ["Qualidade visual premium","Seriedade técnica"],',
      '      "esta_perto": ["Consultoria estratégica (hoje dependente da liderança)"],',
      '      "esta_longe": ["Agilidade digital","Autonomia do time médio"],',
      '      "ninguem_entrega": ["Comunicação interna com o mesmo rigor do externo"]',
      '    },',
      '    "marca_categoria": {',
      '      "categoria_atual": "Produtora que Pensa",',
      '      "categoria_almejada": "Parceira de Inteligência de Comunicação"',
      '    },',
      '    "posicao_competitiva": {',
      '      "referencias_admiradas": ["Marcas que equilibram clareza e estética"],',
      '      "concorrentes_reais": ["Agências digital-native (Piloto, Mojo)"],',
      '      "substitutos": ["Freelancers e agências regionais"]',
      '    },',
      '    "dados_quantitativos": [',
      '      { "metrica": "NPS médio", "valor": 7.0, "leitura": "Zona de Aperfeiçoamento (ressalva: N=5)" },',
      '      { "metrica": "Qualidade de Acabamento (média)", "valor": 8.8, "leitura": "Ativo central" }',
      '    ],',
      '    "zona_simbolica_emocional": "A marca é associada ao Cuidado e à Soberania Visual…",',
      '    "ida_ve": {',
      '      "impulsionadores": ["Rigor editorial — olhar do diretor visto como insubstituível"],',
      '      "detratores": ["Bicefalia operacional — percepção de duas empresas que não se falam"],',
      '      "aceleradores": ["Comunicação interna e cultura — demanda reprimida em 3 clientes"]',
      '    },',
      '    "hipoteses_direcionais": [',
      '      {',
      '        "dimensao": "negocio",',
      '        "hipotese": "A VE sugere que a marca precisa decidir se é uma Boutique Audiovisual ou uma Agência Integrada — tentar ser ambos com a estrutura atual gera o NPS 5 em clientes dinâmicos."',
      '      }',
      '    ]',
      '  },',
      '  "_gaps": []',
  ...
  getUserPrompt(context) {
    ...
    clientes.forEach((f, i) => {
      ...
      const nome = r._respondente_nome || f.respondente || `Cliente ${i + 1}`;
