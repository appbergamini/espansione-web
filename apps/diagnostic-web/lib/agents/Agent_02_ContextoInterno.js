...
export const Agent_02_ContextoInterno = {
  ...
  getSystemPrompt() {
    return [
      ...
      // Particularidade do Agente 2: este é o agente FUNDACIONAL da Brand Memory pro
      // que diz respeito a perfil de equipe, cultura comportamental e maturidade
      // organizacional. O Agent_14 (EVP) consome este export via Output 2. Anonimato
      // ortodoxo dos colaboradores DEVE ser preservado também no JSON — clusters,
      // nunca nomes. Sócios mantêm nomes literais (sem adicionar sobrenomes).
      'EXPORTAÇÃO PARA BRAND MEMORY',
      '',
      'Após os blocos XML acima, emita uma seção adicional <brand_memory_export> com JSON estruturado que o Agente 16 vai consolidar na Brand Memory. Esta seção NÃO substitui nada do output existente — é aditiva. Este é o agente fundacional da Brand Memory pra equipe + cultura + maturidade — todo cuidado.',
      '',
      'REGRAS:',
      '- Emita exclusivamente JSON válido entre as tags. Sem comentários, sem markdown, sem texto antes ou depois do `{`.',
      '- schema_version sempre "2.0", agent_id sempre 2.',
      '- caveat_amostra: texto explícito sobre cobertura da escuta (CIS, 360°, N de respondentes) — exatamente como você descreveu no topo do <conteudo>. null se não há limitação significativa.',
      '- socios_disc_profiles: array com 1 objeto por sócio respondente. nome BYTE-A-BYTE do input (sem adicionar sobrenomes — regra de fidelidade do <conteudo>). disc_profile com scores D/I/S/C e dominante. jung_type (string ou null). estilo_lideranca (Executivo/Motivador/Metodico/Sistematico ou null).',
      '- cultura_time é OPCIONAL — só preencher se cobertura do CIS coletivo ≥70%. Se <70%, emitir cultura_time: null e adicionar a _gaps. Quando preencher, INCLUIR AS 16 competências TODAS no heatmap_competencias (mesmo as fracas) — chaves canônicas snake_case, scores 0-100.',
      '- maturidade_360 é OPCIONAL — só preencher se cobertura ≥80%. Os 6 pilares são FIXOS: estrategia, financas, comercial, marketing, pessoas, operacao (minúsculo, sem acento). Cada pilar com score_medio (0-100), prioridade ("alta" | "media" | "baixa"), divergencia_socios (boolean — true se desvio padrão entre sócios >15pp, false caso contrário).',
      '- cultura_vivida_gaps: cada dimensão cultural da Seção 5 vira { dimensao, leitura_socios, leitura_colaboradores, status }. status: "alinhado" | "parcialmente_alinhado" | "desalinhado".',
      '- posicionamento_aspiracional_vs_real: cada sócio que fez o teste T&W vira um objeto. leitura é uma de: "declarado_igual_operado" | "aspiracional" | "inconsistencia" | "divergencia_entre_socios".',
      '- ambicao_vs_capacidade.gaps_criticos: array ORDENADO do mais crítico ao menos crítico (máx 3 itens). Cada gap com ambicao_declarada, capacidade_atual, criticidade ("alta" | "media" | "baixa").',
      '- tensoes_criticas: cada tensão da Seção 8 vira objeto. tipo: "discurso_vs_pratica" | "socios_vs_colaboradores" | "socio_vs_socio" | "presente_vs_futuro" | "marca_vs_cultura" | "estrategia_vs_estrutura".',
      '- ida_vi: 3 arrays (impulsionadores, detratores, aceleradores). Cada item com descricao + evidencia (citação ou referência ao formulário/entrevista).',
      '- hipoteses_revisadas: cada hipótese do Agente 1 da Seção 10.1 vira objeto. status: "confirmada" | "refutada" | "refinada". formulacao_nova é obrigatório se status="refinada", null caso contrário.',
      '- hipoteses_novas: hipóteses emergentes da Seção 10.2 (array — pode estar vazio).',
      '- risco_concentracao_competencias: APENAS competências concentradas em 1-2 pessoas (Seção 3.3). Cada uma com competencia (chave snake_case), area (cluster amplo, sem nome), risco_operacional.',
      '- enps: OPCIONAL — se calculado, emitir { valor, classificacao: "zona_critica" | "zona_aperfeicoamento" | "zona_qualidade" | "zona_excelencia" }. null se não calculado.',
      '- evidencias_literais: 5-10 citações fundacionais (sem identificação individual nos casos de colaboradores) que outros agentes podem reutilizar. Cada uma com citacao, fonte ("socio:Nome" | "cluster:descricao"), categoria.',
      '- Strings em português, byte-a-byte como você as escreveu no <conteudo>.',
      '',
      'CAMPOS OBRIGATÓRIOS MÍNIMOS:',
      '- socios_disc_profiles: pelo menos 1 sócio com CIS',
      '- ida_vi: 3 chaves (impulsionadores, detratores, aceleradores) — arrays podem estar vazios mas as chaves existem',
      '- tensoes_criticas: pelo menos 1 item (a VI sempre revela alguma tensão)',
      '',
      'FORMATO:',
      '',
      '<brand_memory_export>',
      ...
      '  "agent_id": 2,',
      '  "vi": {',
      '    "caveat_amostra": "A leitura interna está consolidada com cobertura plena de DISC dos 2 sócios. A escuta dos colaboradores cobriu 24% do time total — leitura coletiva limitada por amostra; markers de visualização do CIS coletivo NÃO foram emitidos.",',
      '    "socios_disc_profiles": [',
      '      {',
      '        "nome": "Rafael Menezes",',
      '        "disc_profile": { "D": 65, "I": 40, "S": 30, "C": 88, "dominante": "C" },',
      '        "jung_type": "ISTJ",',
      '        "estilo_lideranca": "Metodico",',
      '        "fortalezas": ["Detalhismo obsessivo (88) sustenta o padrão de grife","Visão estratégica conjugada com execução tática"],',
      '        "pontos_atencao": ["Centralização de aprovações cria gargalo","Dificuldade em delegar a aprovação estética para o time"]',
      '      },',
      '      {',
      '        "nome": "Laura Costa",',
      '        "disc_profile": { "D": 78, "I": 70, "S": 25, "C": 50, "dominante": "D" },',
      '        "jung_type": "ENTJ",',
      '        "estilo_lideranca": "Executivo",',
      '        "fortalezas": ["Energia comercial alta (I=70) impulsiona prospecção","Decisão rápida (D=78) em momentos de pressão"],',
      '        "pontos_atencao": ["Velocidade pode atropelar o ritmo do parceiro detalhista","Baixa paciência (S=25) com processos longos do core audiovisual"]',
      '      }',
      '    ],',
      '    "cultura_time": {',
      '      "disc_predominante": "S",',
      '      "jung_distribuicao": { "I": 7, "E": 3, "S": 8, "N": 2, "T": 6, "F": 4, "J": 7, "P": 3 },',
      '      "caracteristicas_narrativas": ["Time executor-aprimorador — opera bem em melhoria de processos","Lealdade longa (média 5 anos de casa) com baixa rotatividade","Tendência ao silêncio técnico em vez de confronto produtivo"],',
      '      "heatmap_competencias": {',
      '        "comunicacao": 72, "lideranca": 55, "execucao": 81, "estrategia": 48,',
      '        "empatia": 38, "negociacao": 60, "criatividade": 78, "analise": 70,',
      '        "trabalho_equipe": 75, "adaptabilidade": 50, "iniciativa": 45, "responsabilidade": 85,',
      '        "resolucao_problemas": 65, "visao_negocio": 52, "orientacao_resultado": 68, "gestao_tempo": 58',
      '      },',
      '      "estilo_lideranca_dominante": "Metodico"',
      '    },',
      '    "maturidade_360": {',
      '      "pilares": [',
      '        { "nome": "estrategia",  "score_medio": 62, "prioridade": "media", "divergencia_socios": false },',
      '        { "nome": "financas",   "score_medio": 75, "prioridade": "baixa", "divergencia_socios": false },',
      '        { "nome": "comercial",  "score_medio": 58, "prioridade": "media", "divergencia_socios": true },',
      '        { "nome": "marketing",  "score_medio": 42, "prioridade": "alta",  "divergencia_socios": false },',
      '        { "nome": "pessoas",    "score_medio": 51, "prioridade": "media", "divergencia_socios": true },',
      '        { "nome": "operacao",   "score_medio": 34, "prioridade": "alta",  "divergencia_socios": true }',
      '      ],',
      '      "pilar_mais_forte": "financas",',
      '      "pilar_mais_fraco": "operacao"',
      '    },',
      '    "cultura_vivida_gaps": [',
      '      {',
      '        "dimensao": "Opinião ouvida",',
      '        "leitura_socios": "Acreditam que cultivam ambiente aberto a sugestões.",',
      '        "leitura_colaboradores": "Cluster Audiovisual descreve medo de discordar do sócio detalhista.",',
      '        "status": "desalinhado"',
      '      }',
      '    ],',
      '    "posicionamento_aspiracional_vs_real": [',
      '      {',
      '        "socio_nome": "Rafael Menezes",',
      '        "vetor_escolhido_teste": "Liderança de Produto",',
      '        "vetor_sugerido_operacao": "Liderança de Produto",',
      '        "leitura": "declarado_igual_operado",',
      '        "implicacoes": "O foco em rigor estético e autoria está coerente com a operação real."',
      '      },',
      '      {',
      '        "socio_nome": "Laura Costa",',
      '        "vetor_escolhido_teste": "Excelência Operacional",',
      '        "vetor_sugerido_operacao": "Intimidade com Cliente",',
      '        "leitura": "divergencia_entre_socios",',
      '        "implicacoes": "Há tensão estratégica de raiz entre os sócios — escolha de vetor pendente."',
      '      }',
      '    ],',
      '    "ambicao_vs_capacidade": {',
      '      "ambicao_declarada": "Crescer para R$ 30M nos próximos 3 anos com posicionamento de Consultoria de Narrativa B2B.",',
      '      "gaps_criticos": [',
      '        {',
      '          "ambicao_declarada": "Operação que escale para 30M sem perder rigor estético",',
      '          "capacidade_atual": "Operação em 34% de maturidade; aprovações centralizadas no sócio detalhista",',
      '          "criticidade": "alta"',
      '        },',
      '        {',
      '          "ambicao_declarada": "Tornar-se Consultoria de Narrativa B2B",',
      '          "capacidade_atual": "Estratégia em 62% e marketing em 42% — sem narrativa metodológica sistematizada",',
      '          "criticidade": "alta"',
      '        }',
      '      ]',
      '    },',
      ...
      '      {',
      '        "tipo": "socio_vs_socio",',
      '        "titulo": "Boutique premium vs Agência de escala",',
      '        "descricao": "Sócio detalhista quer manter operação boutique com margem alta; sócia comercial quer escalar com gerência média.",',
      '        "evidencia_lado_a": "Não aceito perder o controle do acabamento (formulário Rafael)",',
      '        "evidencia_lado_b": "Sem escalar não chego nos 30M (formulário Laura)",',
      '        "por_que_importa": "A divergência raiz precisa ser equacionada antes da Plataforma de Branding — duas estratégias mutuamente excludentes hoje."',
      '      },',
      '      {',
      '        "tipo": "discurso_vs_pratica",',
      '        "titulo": "Cultura aberta vs medo de discordar",',
      '        "descricao": "Sócios acreditam que o ambiente é aberto a sugestões; cluster Audiovisual relata o oposto.",',
      '        "evidencia_lado_a": "Sócios: nosso time fala abertamente",',
      '        "evidencia_lado_b": "Cluster Audiovisual: a gente espera o humor do dia antes de propor algo",',
      '        "por_que_importa": "Gap de segurança psicológica trava a inovação no core audiovisual e cria risco de fuga de talento."',
      '      }',
      '    ],',
      '    "ida_vi": {',
      '      "impulsionadores": [',
      '        { "descricao": "Rigor estético inegociável sustentado pela liderança", "evidencia": "Pilar de Detalhismo C=88 no sócio + heatmap competencias com execução 81" },',
      '        { "descricao": "Lealdade longa do time (média 5 anos de casa)", "evidencia": "Bloco 1 de tempo de casa dos colaboradores + ausência de turnover declarado" }',
      '      ],',
      '      "detratores": [',
      '        { "descricao": "Centralização de aprovações nos sócios trava a operação", "evidencia": "Cluster Atendimento Sênior: tudo precisa passar pelo Rafael (entrevista)" },',
      '        { "descricao": "Marketing em 42% sem narrativa metodológica sistematizada", "evidencia": "Diagnóstico 360° pilar Marketing = 42; 0% dos sócios marcou metodologia explícita" }',
      '      ],',
      '      "aceleradores": [',
      '        { "descricao": "Time com perfil executor-aprimorador é solo fértil pra implantação de Playbook", "evidencia": "DISC predominante S + heatmap responsabilidade 85 + característica narrativa executor-aprimorador" }',
      '      ]',
      '    },',
      '    "hipoteses_revisadas": [',
      '      {',
      '        "hipotese_original": "Os sócios estão alinhados sobre o crescimento futuro",',
      '        "status": "refutada",',
      '        "evidencia": "Teste T&W revelou vetores divergentes; entrevistas confirmaram a tensão Boutique vs Escala.",',
      '        "formulacao_nova": null',
      '      },',
      '      {',
      '        "hipotese_original": "A cultura comportamental sustenta o posicionamento premium",',
      '        "status": "refinada",',
      '        "evidencia": "Sustenta o rigor estético, mas não sustenta a velocidade comercial.",',
      '        "formulacao_nova": "A cultura sustenta o pilar de Produto (Onda 1) mas não sustenta a operação comercial necessária para escala."',
      '      }',
      '    ],',
      '    "hipoteses_novas": [',
      '      "O risco de concentração de aprovação no sócio detalhista é o gargalo crítico que trava qualquer rota de crescimento."',
      '    ],',
      '    "risco_concentracao_competencias": [',
      '      {',
      '        "competencia": "empatia",',
      '        "area": "Cluster Atendimento Sênior (1 pessoa)",',
      '        "risco_operacional": "Conexão emocional com clientes concentrada em 1 pessoa; se sair, 3 contas-chave perdem o vínculo."',
      '      },',
      '      {',
      '        "competencia": "visao_negocio",',
      '        "area": "Sociedade (1 sócio)",',
      '        "risco_operacional": "Aprovação estética e estratégica centralizada — qualquer indisponibilidade do sócio detalhista trava entregas."',
      '      }',
      '    ],',
      '    "enps": { "valor": 18, "classificacao": "zona_aperfeicoamento" },',
      '    "evidencias_literais": [',
      '      { "citacao": "Não aceito perder o controle do acabamento.", "fonte": "socio:Rafael Menezes", "categoria": "diferencial_declarado" },',
      '      { "citacao": "A gente espera o humor do dia antes de propor algo.", "fonte": "cluster:Audiovisual 2-5 anos", "categoria": "cultura_vivida" }',
      '    ]',
      '  },',
      '  "_gaps": []',
      '}',
