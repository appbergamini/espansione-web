...
function drawCover(doc, out, projetoNome) {
  ...
  // FIX.23 — capa NÃO repete o resumo executivo. Antes, o mesmo texto
  // de out.resumo_executivo aparecia aqui (até 6 linhas) E no card da
  // página 2, gerando redundância visual. Capa fica limpa: título +
  // subtítulo + meta.
...
function drawMarkdownTable(doc, headers, rows, mx, startY, maxW, pageH, onNewPage) {
  ...
  // Linhas com altura dinâmica (células longas quebram em múltiplas linhas)
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(fontSize);
  rows.forEach((row, rIdx) => {
    // Pré-calcula as linhas de cada célula e a altura máxima da linha
    const cellsLines = row.map((cell, cIdx) => {
      if (cIdx >= cols) return [''];
      return doc.splitTextToSize(stripInline(cell), colW - 2 * cellPadX);
    });
    const nLines = Math.max(1, ...cellsLines.map(l => l.length));
    const rowH = Math.max(18, nLines * lineHeight + cellPadY * 2 - lineHeight * 0.5);
    ensure(rowH);
    ...
    row.forEach((cell, cIdx) => {
      ...
      if (hasBold) {
        // renderInlineLine para preservar bold — limite 1 linha por célula bold
        doc.setTextColor(40, 40, 40);
        renderInlineLine(doc, cell, x, y + cellPadY + fontSize, colW - 2 * cellPadX, { size: fontSize, color: [40, 40, 40] });
      } else {
        rgb(doc, [40, 40, 40]);
        const lines = cellsLines[cIdx];
        lines.forEach((ln, idx) => {
          doc.text(ln, x, y + cellPadY + fontSize + idx * lineHeight);
        });
      }
    });
